...
class FacebookExtensionTest extends \PHPUnit_Framework_TestCase
{
  ...
  public function testRenderInitialize( )
  {
    ...
    $helperMock->expects( $this->once( ) )->method( 'initialize' )->will( $this->returnValue( 'returnedValue' ) );
    $containerMock = $this->getMock( 'Symfony\Component\DependencyInjection\ContainerInterface' );
    $containerMock->expects( $this->once( ) )->method( 'get' )->with( 'fos_facebook.helper' )
  ...
  public function testRenderLoginButton( )
  {
    $helperMock = $this->getMockBuilder( 'FOS\FacebookBundle\Templating\Helper\FacebookHelper' )
        ->disableOriginalConstructor( )->getMock( );
    $helperMock->expects( $this->once( ) )->method( 'loginButton' )->will( $this->returnValue( 'returnedValueLogin' ) );
    $containerMock = $this->getMock( 'Symfony\Component\DependencyInjection\ContainerInterface' );
    $containerMock->expects( $this->once( ) )->method( 'get' )->with( 'fos_facebook.helper' )
        ->will( $this->returnValue( $helperMock ) );
