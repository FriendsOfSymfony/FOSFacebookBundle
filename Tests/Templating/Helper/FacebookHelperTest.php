...
class FacebookHelperTest extends \PHPUnit_Framework_TestCase
{
  ...
  public function testInitialize( )
  {
    ...
    $templating->expects( $this->once( ) )->method( 'render' )
        ->with( 'FOSFacebookBundle::initialize.html.php',
            array( 'appId' => 123, 'async' => true, 'cookie' => false, 'culture' => 'en_US', 'fbAsyncInit' => '',
                'logging' => true, 'oauth' => true, 'status' => false, 'xfbml' => false, ) )
        ->will( $this->returnValue( $expected ) );
    
    $facebookMock = $this->getMockBuilder( 'FOS\FacebookBundle\Facebook\FacebookSessionPersistence' )
        ->disableOriginalConstructor( )->setMethods( array( 'getAppId' ) )->getMock( );
    $facebookMock->expects( $this->once( ) )->method( 'getAppId' )->will( $this->returnValue( '123' ) );
  ...
  public function testLoginButton( )
  {
    ...
    $templating = $this->getMockBuilder( 'Symfony\Component\Templating\DelegatingEngine' )
        ->disableOriginalConstructor( )->getMock( );
    $templating->expects( $this->once( ) )->method( 'render' )
        ->with( 'FOSFacebookBundle::loginButton.html.php',
            array( 'autologoutlink' => 'false', 'label' => 'testLabel', 'scope' => '1,2,3', ) )
        ->will( $this->returnValue( $expected ) );
