...
class FacebookListenerTest extends \PHPUnit_Framework_TestCase
{
  ...
  {
    $listener = new FacebookListener( $this->getMock( 'Symfony\Component\Security\Core\SecurityContextInterface' ),
        $this->getAuthenticationManager( ),
        $this->getMock( 'Symfony\Component\Security\Http\Session\SessionAuthenticationStrategyInterface' ),
        $this->getHttpUtils( ), 'providerKey',
  ...
  {
    $authenticationManagerMock = $this
  ...
  private function getHttpUtils( )
  {
    ...
    $httpUtils->expects( $this->once( ) )->method( 'checkRequestPath' )->will( $this->returnValue( true ) );
  ...
  {
    $responseEventMock = $this
        ->getMock( 'Symfony\Component\HttpKernel\Event\GetResponseEvent', array( 'getRequest' ), array( ), '', false );
    $responseEventMock->expects( $this->any( ) )->method( 'getRequest' )->will( $this->returnValue( $this->getRequest( ) ) );
  ...
  {
    $requestMock = $this->getMockBuilder( 'Symfony\Component\HttpFoundation\Request' )->disableOriginalClone( )
        ->getMock( );
    $requestMock->expects( $this->any( ) )->method( 'hasSession' )->will( $this->returnValue( 'true' ) );
    $requestMock->expects( $this->any( ) )->method( 'hasPreviousSession' )->will( $this->returnValue( 'true' ) );
