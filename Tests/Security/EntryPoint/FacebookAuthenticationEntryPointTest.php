...
class FacebookAuthenticationEntryPointTest extends \PHPUnit_Framework_TestCase
{
  ...
  public function testThatRedirectResponseWithFacebookLoginUrlIsCreated( )
  {
    ...
    $requestMock->expects( $this->once( ) )->method( 'getUriForPath' )->with( $this->equalTo( '/index' ) )
    ...
    $facebookMock = $this->getMockBuilder( 'FOS\FacebookBundle\Facebook\FacebookSessionPersistence' )
        ->disableOriginalConstructor( )->setMethods( array( 'getLoginUrl' ) )->getMock( );
    $facebookMock->expects( $this->once( ) )->method( 'getLoginUrl' )
        ->with( 
            $this
                ->equalTo( 
                    array( 'display' => 'page', 'scope' => 'email,user_website',
                        'redirect_uri' => 'http://localhost/index' ) ) )
        ->will( $this->returnValue( 'http://localhost/facebook-redirect/index' ) );
    
    $facebookAuthentication = new FacebookAuthenticationEntryPoint( $facebookMock, $options,
        array( 'email', 'user_website' ));
    ...
    $this
        ->assertEquals( $response->headers->get( 'location' ), 'http://localhost/facebook-redirect/index',
            'RedirectResponse has defined expected location' );
  }
  ...
  public function testThatRedirectionToFacebookLoginUrlIsCreated( )
  {
    ...
    $this
        ->assertRegExp( '/location\.href="http:\/\/localhost\/facebook-redirect\/index/', $response->getContent( ),
            'Javascript redirection is in response' );
  }
}
