...
class FacebookFactoryTest extends \PHPUnit_Framework_TestCase
{
  ...
  {
    $idsArray = $this
        ->facebookFactoryCreate( 
            array( 'provider' => true, 'remember_me' => false, 'create_user_if_not_exists' => false ) );
  ...
  {
    $definition = $this
        ->getMock( 'Symfony\Component\DependencyInjection\Definition', array( 'addArgument', 'replaceArgument' ) );
    $definition->expects( $this->any( ) )->method( 'replaceArgument' )->will( $this->returnValue( $definition ) );
    $definition->expects( $this->any( ) )->method( 'addArgument' )->will( $this->returnValue( $definition ) );
    $container = $this->getMock( 'Symfony\Component\DependencyInjection\ContainerBuilder', array( 'setDefinition' ) );
    $container->expects( $this->any( ) )->method( 'setDefinition' )->will( $this->returnValue( $definition ) );
